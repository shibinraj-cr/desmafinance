...
import { getSubscriptionStatus, setSubscriptionExpiry } from "@/lib/etimeoffice-subscription";
...
export const GET = withApiHandler(async () => {
  ...
  return NextResponse.json({
    ...
    // Subscription renewal (entered manually — no API for it) + live countdown.
    subscription: await getSubscriptionStatus(),
  });
});

const PostSchema = z.object({
  action: z.enum(["save", "test", "clear", "save-subscription"]),
  ...
  expiry: z.string().trim().max(10).optional(), // YYYY-MM-DD subscription renewal date
});
...
export const POST = withApiHandler(async (req: Request) => {
  ...
  if (body.action === "save-subscription") {
    const ok = await setSubscriptionExpiry((body.expiry ?? "").trim(), userId);
    if (!ok) throw badRequest("Enter a valid renewal date (YYYY-MM-DD)", "bad_expiry");
    return NextResponse.json({ ok: true, subscription: await getSubscriptionStatus() });
  }
