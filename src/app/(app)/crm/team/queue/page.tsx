...
function parseIssue(v: string | undefined): AttentionIssue | null {
  return v && (ISSUE_VALUES as string[]).includes(v) ? (v as AttentionIssue) : null;
}
...
export default async function AttentionQueuePage({ searchParams }: { searchParams: SP }) {
  ...
  // Consultant roster for the dropdown (narrowed to self for a plain BDE).
  ...
  const now = new Date();
  const { rows: allRows, counts } = await getAttentionQueue({ scope, consultantId, now });
  const rows = issue ? allRows.filter((r) => rowHasIssue(r, issue)) : allRows;
  ...
  const issueHref = (v: AttentionIssue | null) => {
      ...
      <div className="p-margin space-y-lg">
        ...
        >
          <div className="overflow-x-auto">
            <table className="w-full min-w-[820px] border-collapse text-label-sm">
              <thead>
                  ...
                  rows.map((r) => {
                    ...
                    return (
                      <tr key={r.id} className="border-b border-outline-variant/60 hover:bg-surface-container-low">
                        ...
                        {showConsultant && (
                          <td className="px-sm py-sm text-on-surface-variant">{r.assigneeName ?? "Unassigned"}</td>
                        )}
                        <td className="px-sm py-sm">
