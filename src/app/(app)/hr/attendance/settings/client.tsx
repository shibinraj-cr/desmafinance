...
export function EtimeSettingsClient() {
  ...
  const [busy, setBusy] = useState<null | "save" | "test" | "clear" | "sub">(null);
  ...
  async function load() {
    ...
    if (r.ok) {
      ...
      setExpiry(d.subscription?.expiry ?? "");
    }
    setLoading(false);
  }
  useEffect(() => {
  ...
  async function save() {
    ...
    setBusy(null);
    if (!r.ok) {
      setMsg({ tone: "err", text: j.message || j.error || "Save failed" });
      return;
    }
    setPassword("");
  ...
  async function test() {
    ...
    const j = await r.json().catch(() => ({}));
    setBusy(null);
    setMsg(r.ok ? { tone: "ok", text: j.message || "Connected." } : { tone: "err", text: j.message || j.error || "Test failed" });
  }
  ...
  async function saveSubscription() {
    ...
    const r = await fetch("/api/hr/attendance/etime-settings", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ action: "save-subscription", expiry }),
    });
    const j = await r.json().catch(() => ({}));
    setBusy(null);
    if (!r.ok) {
      setMsg({ tone: "err", text: j.message || j.error || "Could not save renewal date" });
      return;
    }
    setMsg({ tone: "ok", text: "Renewal date saved." });
    await load();
  }

  const subTone =
    cfg?.subscription?.tone === "expired"
      ? "bg-red-50 text-red-800 border-red-200"
      : cfg?.subscription?.tone === "warn"
    ...
    <section className={card + " p-lg space-y-md max-w-3xl"}>
      ...
      {cfg && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-md">
            ...
            <div className="sm:col-span-2">
              ...
              <select className={inputCls} value={authMode} onChange={(e) => setAuthMode(e.target.value)}>
                <option value="corp-user-pass">Basic base64(CorpId:Username:Password:True) — default</option>
          ...
          {/* Subscription renewal — entered manually (no API), drives the
              attendance-screen countdown + Admin/HR renewal alerts (7 & 1 days). */}
          <div className="border-t border-outline-variant pt-md space-y-sm">
            <div className="flex items-center justify-between gap-base">
              <h4 className="text-label-lg font-semibold text-on-surface">Subscription renewal</h4>
              {cfg.subscription?.daysLeft != null && (
                <span className={"px-sm py-[2px] rounded-full text-[11px] font-bold border " + subTone}>
                  {cfg.subscription.label}
                </span>
              )}
            </div>
            <p className="text-caption text-on-surface-variant">
              eTimeOffice has no API for the account expiry, so enter the renewal date from the Team Office
              portal (Order History). Admins &amp; HR are notified 7 and 1 days before it lapses.
            </p>
            <div className="flex flex-wrap items-end gap-base">
              ...
              <button className={primary} disabled={busy !== null || !expiry} onClick={saveSubscription}>
                {busy === "sub" ? "Saving…" : "Save renewal date"}
              </button>
            </div>
          </div>
          ...
          <p className="text-caption text-on-surface-variant border-t border-outline-variant pt-md">
            ...
            page for an immediate pull, or let the scheduled cron keep it current. The Corporate ID, Username
