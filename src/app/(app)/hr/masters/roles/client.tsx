...
type Row = { id: string; name: string; description: string | null; active: boolean; members: number };

export function RolesEditor({ rows, canEdit }: { rows: Row[]; canEdit: boolean }) {
  ...
  async function saveEdit(id: string) {
    const res = await fetch(`/api/hr/roles/${id}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ ...editVals, description: editVals.description || null }),
    });
    if (res.ok) {
      setEditing(null);
      start(() => router.refresh());
  ...
  async function remove(id: string) {
    if (!confirm("Delete this role? (If employees use it, it will be deactivated.)")) return;
        ...
        <table className="w-full text-label-sm">
          <thead className="text-left text-on-surface-variant border-b border-outline-variant">
            <tr>
              ...
              <th className="py-sm pr-md">Description</th>
                    ...
                    {editing === r.id ? (
                      ...
                      <>
                        <button
                          onClick={() => {
                            setEditing(r.id);
                            setEditVals({ name: r.name, description: r.description ?? "" });
                          }}
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={canEdit ? 5 : 4} className="py-lg text-center text-on-surface-variant">
                  No roles yet. Click + Add role above.
                </td>
              </tr>
            )}
          </tbody>
