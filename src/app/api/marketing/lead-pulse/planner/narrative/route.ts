...
import { generatePlanNarrative } from "@/lib/lead-pulse-planner-ai";
...
const Body = z.object({
  ...
  // Slider overrides (the levers the user can move); everything else stays
  // at the historic baseline. The server recomputes the plan from these so it
  // never trusts the client's arithmetic.
...
export async function POST(req: NextRequest) {
  ...
  const result = await generatePlanNarrative({ ...baseline, rates }, plan);
