...
export default async function DailyTrackerPage({
  ...
  let running = 0;
  // We'll display newest first but compute running balance chronologically.
  const chronological = [...items].sort((a, b) => +a.date - +b.date || +a.createdAt - +b.createdAt);
  ...
  return (
    <>
      <TopBar title="Daily Tracker" subtitle={`${items.length} transactions`} />
      <div className="p-margin space-y-lg">
        <div className="flex flex-wrap gap-base items-center">
          <FilterChip href="/daily-tracker" active={!searchParams.type && !searchParams.month}>All</FilterChip>
          <FilterChip href="/daily-tracker?type=Revenue" active={searchParams.type === "Revenue"}>Inflow</FilterChip>
          <FilterChip href="/daily-tracker?type=Expense" active={searchParams.type === "Expense"}>Outflow</FilterChip>
          <Link
            href="/daily-tracker/new"
            className="ml-auto inline-flex items-center gap-xs h-9 px-md rounded-lg bg-primary text-on-primary text-label-sm font-semibold hover:bg-primary-container transition"
          >
                ...
                {items.length === 0 && (
                  <tr>
                    <td colSpan={10} className="p-lg text-center text-on-surface-variant">
                      No transactions yet. Click <strong>New transaction</strong> to record one.
                    </td>
                  </tr>
                )}
                {items.map((t) => {
