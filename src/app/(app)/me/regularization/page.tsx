...
export default async function MyRegularizationPage({
  ...
  const exceptions = days
    .filter((d) => {
      ...
      if (requested.has(ds)) return false;
      const punches = (d.inTime ? 1 : 0) + (d.outTime ? 1 : 0);
      const incomplete = punches === 1; // exactly one of in/out recorded
      return d.status === "A" || incomplete;
    })
    .map((d) => ({
      ...
      in: d.inTime,
      out: d.outTime,
      kind: d.status === "A" ? ("absent" as const) : ("incomplete" as const),
    }));
  ...
  return (
    <>
      <TopBar
        title="My Attendance"
        subtitle={`Raise punch / leave requests within ${REGULARIZATION_WINDOW_WORKING_DAYS} working days · ${exceptions.length} to review`}
      />
      <div className="p-margin space-y-lg">
        <RegularizationRequestClient
          ...
          requests={allRequests.map((r) => ({
            ...
            reasonLabel: r.requestType === "leave" ? "Leave request" : reasonLabel[r.reasonType] ?? r.reasonType,
            reason: r.reason,
