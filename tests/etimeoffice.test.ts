...
describe("buildAuthHeader", () => {
  it("defaults to Basic base64(corp:user:pass)", () => {
    const h = buildAuthHeader(baseCfg);
    expect(h).toBe(`Basic ${Buffer.from("DESMA:admin:secret").toString("base64")}`);
  });
...
describe("mapPunchRecord", () => {
  ...
  it("treats a single punch as a half-day pending regularization", () => {
    const row = mapPunchRecord({
      Empcode: "7",
      Name: "Sivapriya",
