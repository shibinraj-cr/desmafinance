    ...
    select: {
      ...
      messages: {
        ...
        select: {
          ...
          mediaMime: true,
          fileName: true,
          // Whether an attachment exists — never the provider's id. The bytes
          // come from /api/crm/wa/media/<messageId>, which authorises per
          // conversation; a media id in the page would be a handle to a file
          // whose only protection was that nobody had seen the number.
          mediaId: true,
          mediaUrl: true,
          templateName: true,
          waStatus: true,
    ...
    conversation: {
      ...
      lead: conversation.lead
        ? {
            ...
            qualificationOther: conversation.lead.qualificationOther,
            subStatusId: conversation.lead.subStatusId,
            subStatusLabel: conversation.lead.subStatus?.label ?? null,
            subStatusSince: conversation.lead.subStatusSince,
