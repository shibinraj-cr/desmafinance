 ...
 *                              corpId:username:password); or "user-pass":
 *                              standard Basic base64(username:password).
 *
 * Confirm the right one against your official eTimeOffice API document before
 * relying on the sync. Everything else here is portal-agnostic.
 */
...
export function buildAuthHeader(cfg: EtimeConfig): string {
  ...
  const cred =
    cfg.authMode === "user-pass"
      ...
      : `${cfg.corpId}:${cfg.username}:${cfg.password}`;
 ...
 * usable date.
 */
export function mapPunchRecord(rec: Record<string, unknown>): ParsedDay | null {
