...
export async function POST() {
  ...
  try {
    ...
    // 8. Collection Plan — Finance Executive stages expected receivables
    //    per candidate as N installments; each can be submitted to the
    //    Daily Tracker for approval which creates a real Transaction.
    await step(
    ...
    await step(
      "CollectionPlan partyId index",
      `CREATE INDEX IF NOT EXISTS "CollectionPlan_partyId_idx" ON "CollectionPlan"("partyId")`,
    );
    await step(
      "CollectionPlan serviceId index",
      `CREATE INDEX IF NOT EXISTS "CollectionPlan_serviceId_idx" ON "CollectionPlan"("serviceId")`,
    );
    await step(
      "CollectionPlan status index",
      `CREATE INDEX IF NOT EXISTS "CollectionPlan_status_idx" ON "CollectionPlan"("status")`,
    );
    await step(
      "CollectionPlan createdById index",
      `CREATE INDEX IF NOT EXISTS "CollectionPlan_createdById_idx" ON "CollectionPlan"("createdById")`,
    );
    await step(
      "CollectionPlan.partyId foreign key",
      `DO $$ BEGIN
    ...
    await step(
      "CollectionPlan.serviceId foreign key",
      `DO $$ BEGIN
    ...
    await step(
      "CollectionPlan.createdById foreign key",
      `DO $$ BEGIN
    ...
    await step(
      "CollectionPlanInstallment table",
      `CREATE TABLE IF NOT EXISTS "CollectionPlanInstallment" (
         "id" TEXT NOT NULL,
    ...
    await step(
      "CollectionPlanInstallment unique (planId, seq)",
      `CREATE UNIQUE INDEX IF NOT EXISTS "CollectionPlanInstallment_planId_seq_key" ON "CollectionPlanInstallment"("planId", "seq")`,
    );
    await step(
      "CollectionPlanInstallment pendingApprovalId unique",
      `CREATE UNIQUE INDEX IF NOT EXISTS "CollectionPlanInstallment_pendingApprovalId_key" ON "CollectionPlanInstallment"("pendingApprovalId")`,
    );
    await step(
      "CollectionPlanInstallment transactionId unique",
      `CREATE UNIQUE INDEX IF NOT EXISTS "CollectionPlanInstallment_transactionId_key" ON "CollectionPlanInstallment"("transactionId")`,
    );
    await step(
      "CollectionPlanInstallment planId index",
      `CREATE INDEX IF NOT EXISTS "CollectionPlanInstallment_planId_idx" ON "CollectionPlanInstallment"("planId")`,
    );
    await step(
      "CollectionPlanInstallment status index",
      `CREATE INDEX IF NOT EXISTS "CollectionPlanInstallment_status_idx" ON "CollectionPlanInstallment"("status")`,
    );
    await step(
      "CollectionPlanInstallment expectedDate index",
      `CREATE INDEX IF NOT EXISTS "CollectionPlanInstallment_expectedDate_idx" ON "CollectionPlanInstallment"("expectedDate")`,
    );
    await step(
      "CollectionPlanInstallment.planId foreign key",
      `DO $$ BEGIN
    ...
    await step(
      "CollectionPlanInstallment.transactionId foreign key",
      `DO $$ BEGIN
    ...
    // PendingApproval.collectionInstallmentId — wires the approval queue
    // back to the staged installment so approve/reject can flip status.
    await step(
      "PendingApproval.collectionInstallmentId column",
      `ALTER TABLE "PendingApproval" ADD COLUMN IF NOT EXISTS "collectionInstallmentId" TEXT`,
    );
    await step(
      "PendingApproval.collectionInstallmentId unique",
      `CREATE UNIQUE INDEX IF NOT EXISTS "PendingApproval_collectionInstallmentId_key" ON "PendingApproval"("collectionInstallmentId")`,
    );
    await step(
      "PendingApproval.collectionInstallmentId foreign key",
      `DO $$ BEGIN
