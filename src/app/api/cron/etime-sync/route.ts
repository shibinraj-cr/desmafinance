...
/**
 * Automatic daily eTimeOffice attendance sync (Vercel Cron → see vercel.json).
 *
...
async function handle(req: Request): Promise<NextResponse> {
  ...
  if (!cfg) {
    return NextResponse.json({ ok: false, skipped: "eTimeOffice not configured" }, { status: 200 });
  }
